...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        for i in range(1, pc):
            ...
            table = soup.find_all('aside', {'class': 'eds-media-card-content__image-container'})  # returns a list
            # table = soup.find(class="eds-media-card-content__image-container")
            # print(table)
            # thead = table.findChild('thead', recursive=False)
            event_id_list = []
            for rw in table:
                trs = rw.findChildren('a', recursive=False)[0]
                link = trs.get('href')
                start = link.rfind('-') + 1
                end = link.rfind('?')
            ...
            for event_id in event_id_list:
                ...
                if r.status_code == 200:
                    ...
                    if event['logo']:
                        ...
                        logo_url = 'https://revealin.com/static/assets/images/no-photo.png'
                    ...
                    new_event.source_website = source_website
                    if price:
